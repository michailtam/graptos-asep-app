...
# Αρχείο για τη μόνιμη αποθήκευση της προόδου
...
# --- ΣΥΝΑΡΤΗΣΕΙΣ ΜΟΝΙΜΗΣ ΑΠΟΘΗΚΕΥΣΗΣ (JSON) ---
def load_progress():
...
def save_progress():
    progress_data = {
        ...
        "current_indices": st.session_state.current_indices,
        "submitted_sections": list(st.session_state.submitted_sections),
...
if "current_indices" not in st.session_state:
    ...
    st.session_state.current_indices = saved_data.get("current_indices", initial_indices)
if "submitted_sections" not in st.session_state:
    st.session_state.submitted_sections = set(saved_data.get("submitted_sections", []))
...
# Αρχικοποίηση του widget state για να μπορούμε να το κλείνουμε προγραμματιστικά
if "timer_toggle_widget" not in st.session_state:
...
def generate_random_test():
    ...
    # ΔΙΟΡΘΩΣΗ: Πλήρης απενεργοποίηση του ρολογιού οπτικά και λειτουργικά
    st.session_state.timer_start = None  
    st.session_state["timer_toggle_widget"] = False
    save_progress()
...
# ΔΙΟΡΘΩΣΗ: Απενεργοποίηση και οπτικό κλείσιμο ρολογιού όταν αλλάζει η κατηγορία στο μενού
...
if st.session_state.active_mode == "regular":
    ...
    if st.sidebar.button("⬅️ Επιστροφή στις Ενότητες", use_container_width=True, type="secondary"):
        ...
        st.session_state.show_preview = False
        st.session_state.timer_start = None  
        st.session_state["timer_toggle_widget"] = False  # ΔΙΟΡΘΩΣΗ: Κλείσιμο διακόπτη
        st.rerun()

# --- ΚΟΥΜΠΙ: ΕΝΑΡΞΗ ΤΕΣΤ ΑΠΟ ΤΗΝ ΑΡΧΗ ---
if st.sidebar.button("🔄 Έναρξη Τεστ από την Αρχή", use_container_width=True, type="secondary"):
    st.session_state.timer_start = None  
    st.session_state["timer_toggle_widget"] = False  # ΔΙΟΡΘΩΣΗ: Κλείνει οριστικά ο διακόπτης στην οθόνη
...
with col_timer_switch:
    # Σύνδεση του widget με το ελεγχόμενο key προκειμένου να κλείνει αυτόματα
